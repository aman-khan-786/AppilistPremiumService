 ...
 * Add Stock Use Case
 */
class AddStockUseCase(private val database: AppDatabase) {
    
    suspend operator fun invoke(stock: Stock): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val stockWithId = if (stock.id.isEmpty()) {
                    stock.copy(id = UUID.randomUUID().toString())
                } else stock
                
                database.stockDao().insertStock(stockWithId.toEntity())
                Result.success(Unit)
            } catch (e: Exception) {
                Result.failure(e)
            }
